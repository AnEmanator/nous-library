...
class Players {
    constructor(dispatch, mods) {
        ...
        // "me" only self-expires early (see below) - we only ever predict off our
        // OWN abnormalities (skill redirects, aspd multipliers), never another
        // player's, so there's no reason to skew anyone else's stored data
        function setEffect(target, id, duration, stacks, source, hitCylinderId) {
            ...
            // we only learn about an abnormality once its real BEGIN packet arrives,
            // already a full ping late, and the matching real END will lag by roughly
            // the same amount again - so waiting for that END to remove it locally
            // means we keep believing it's active for up to a full ping after the
            // server itself has already dropped it. Self-expire a ping early instead
            // of waiting for the packet - same fix NGSP's own abnormals.js uses
            // (`duration - ping.min`), not guessed here, though we haven't directly
            // confirmed live that this specific staleness is what caused any one
            // consumer-side bug (see Powerlink's combo-chain investigation).
